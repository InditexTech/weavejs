...
import { WeaveUserPointer, WeaveUserPointerKey, WeaveUsersPointersPluginParams } from "./types";
import { WEAVE_USER_POINTER_KEY, WEAVE_USERS_POINTERS_LAYER_ID } from "./constants";
import { WeavePlugin } from "@/plugins/plugin";
import Konva from "konva";
import { Vector2d } from "konva/lib/types";

export class WeaveUsersPointersPlugin extends WeavePlugin {
  private usersPointers: Record<string, { oldPos: WeaveUserPointer; actualPos: WeaveUserPointer }>;
  private usersPointersTimers: Record<string, number>;
  private getUser: () => WeaveUser;
  private usersPointersLayer!: Konva.Layer;
  render: undefined;
  ...
  init() {
    ...
    store.onAwarenessChange((changes: WeaveAwarenessChange<WeaveUserPointerKey, WeaveUserPointer>[]) => {
      ...
      for (const change of changes) {
        ...
        if (change[WEAVE_USER_POINTER_KEY] && selfUser.name !== change[WEAVE_USER_POINTER_KEY].user) {
          ...
          this.usersPointers[userPointer.user] = {
            oldPos: this.usersPointers[userPointer.user]?.actualPos ?? { user: userPointer.user, x: 0, y: 0 },
  ...
  private setUserMovementTimer(userPointer: WeaveUserPointer) {
    ...
    this.usersPointersTimers[userPointer.user] = setTimeout(() => {
      const userPointerNode = this.usersPointersLayer.findOne(`#${userPointer.user}`) as Konva.Group | undefined;
  ...
  private renderPointers() {
    for (const userPointerKey of Object.keys(this.usersPointers)) {
      ...
      const userPointerNode = this.usersPointersLayer.findOne(`#${userPointer.actualPos.user}`) as
        | Konva.Group
        | undefined;
      ...
      if (!userPointerNode) {
        ...
        this.usersPointersLayer.add(userPointerNode);

        this.setUserMovementTimer(userPointer.actualPos);
        continue;
      }

      const oldPos: Vector2d = { x: userPointer.oldPos.x, y: userPointer.oldPos.y };
      const actualPos: Vector2d = { x: userPointer.actualPos.x, y: userPointer.actualPos.y };

import { WeaveSelection, GroupSerializable, NodeSerializable } from "@/types";
...
export class WeaveNodesSelectionPlugin extends WeavePlugin {
  ...
  private getSelectionLayer() {
    const stage = this.instance.getStage();
    const layer = stage.findOne(`#${this.getLayerName()}`) as Konva.Layer;
    return layer;
  }

  private triggerSelectedNodesEvent() {
    const selectedNodes: WeaveSelection[] = this.tr
      .getNodes()
      .map((node) => ({ konvaNode: node, node: node.getAttrs() as NodeSerializable }));
  ...
  private initEvents() {
    ...
    stage.container().addEventListener("keydown", (e) => {
      if (e.key === "Backspace" || e.key === "Delete") {
        ...
        for (const node of selectedNodes) {
          this.instance.removeElement(node.getAttrs() as NodeSerializable | GroupSerializable);
        }
        return;
      }
    });
    ...
    stage.on("click tap", (e) => {
      ...
      if (!nodeToAdd.getAttrs().type) {
        return;
      }

      // console.log({ selectedGroup, target: e.target, nodeToAdd });

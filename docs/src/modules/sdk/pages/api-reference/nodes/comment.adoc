...
The https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/nodes/comment/comment.ts[WeaveCommentNode]
...
| mono:[config] | mono:[Pick<WeaveCommentNodeConfig<T>, 'model' \| 'formatDate' \| 'createComment' \| 'viewComment'> & DeepPartial<Pick<WeaveCommentNodeConfig<T>, 'style'>>] | label:green[Required] | Config parameters for the node.
|===
...
| mono:[style] | mono:[WeaveCommentNodeStyle] |  | mono:[check default values] | Defines the node canvas UI style.
| mono:[model] | mono:[WeaveCommentNodeModel<T>] | label:green[Required] |  | Setup the Entity model 
| mono:[formatDate] | mono:[(date: string) => string] | label:green[Required] |  | A function that formats the date when used on the canvas UI.
| mono:[createComment] | mono:[(ele: HTMLDivElement, node: WeaveElementInstance, finish: (node: WeaveElementInstance, content: string, action: WeaveCommentNodeCreateAction) => void) => Promise<void>] | label:green[Required] |  | A callback that is called when the create comment overlay UI is needed to be render.
| mono:[viewComment] | mono:[(ele: HTMLDivElement, node: WeaveElementInstance, finish: (node: WeaveElementInstance, content: string, action: WeaveCommentNodeViewAction) => void) => Promise<void>] | label:green[Required] |  | A callback that is called when the view / manage comment overlay UI is needed to be render.
|===
...
This method associates to a node instance it respective model instance, useful after creating, to associate
the newly created module to the created instance.
...
Used internally on the xref:main:build/actions/comment-tool.adoc[Comment tool] to handle it

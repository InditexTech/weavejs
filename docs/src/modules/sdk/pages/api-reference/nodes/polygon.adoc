...
The https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/nodes/polygon/polygon.ts[WeavePolygonNode]
...
| mono:[config] | mono:[Partial<WeavePolygonProperties>] |  | Config parameters for the node.
|===
...
| mono:[transform] | mono:[object] |  | mono:[check default values] | Setup the transform properties for the polygon (if it can be resized, rotated, which anchors are available, etc.).
|===

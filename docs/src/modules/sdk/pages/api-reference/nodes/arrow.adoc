...
The https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/nodes/arrow/arrow.ts[WeaveArrowNode]
class represents a basic arrow shape within the Weave.js canvas.

Built on top of Konva’s https://konvajs.org/api/Konva.Arrow.html[Arrow] class shape under the hood, the WeaveArrowNode
...
| mono:[config] | mono:[Partial<WeaveArrowProperties>] | label:green[Required] | Config parameters for the node.
|===
...
| mono:[transform] | mono:[object] |  | mono:[check default values] | Setup the transform properties for the arrow (if can be resized, rotated, anchors, etc.).
|===

= WeaveVideoNode
:description: Video node API Reference

== Overview

The https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/nodes/video/video.ts[WeaveVideoNode]
class represents a video element on the Weave.js collaborative canvas. It allows users to upload display, and
play videos within a real-time shared environment, making it a key building block for visual storytelling,
design workspaces, and multimedia collaboration.

Built on top of Konva’s https://konvajs.org/api/Konva.Image.html[Image] class shape under the hood, and the the
https://konvajs.org/api/Konva.Animation.html[Animation] class, WeaveVideoNode handles video loading (via callbacks),
scaling, positioning, and real-time synchronization, ensuring a smooth collaborative experience even when working
with dynamic media content.

Videos are ideal for:

* Building rich multimedia design spaces or creative whiteboards.
* Enhancing collaborative diagrams with visual context and assets.

The class extends the xref:sdk:api-reference/nodes/node.adoc[WeaveNode] class

== Type

This node `nodeType` property value is `video`.

== Import

[source,shell]
----
import { WeaveVideoNode } from "@inditextech/weave-sdk";
----

== Instantiation

[source,ts]
----
new WeaveVideoNode(params?: WeaveVideoNodeParams);
----

== TypeScript types

[source,ts]
----
type VideoProps = WeaveElementAttributes & {
  id: string;
  width: number;
  height: number;
  videoURL: string;
  videoPlaceholderURL: string;
  videoInfo?: {
    width: number;
    height: number;
  };
};

type WeaveVideoState = {
  placeholderLoaded: boolean;
  loaded: boolean;
  playing: boolean;
  paused: boolean;
};

type VideoBackgroundStyle = {
  color: string;
  strokeWidth: number;
  strokeColor: string;
};

type VideoIconBackgroundStyle = {
  color: string;
  strokeWidth: number;
  strokeColor: string;
};

type VideoTrackStyle = {
  resetOnEnd: boolean;
  onlyOnHover: boolean;
  color: string;
  height: number;
};

type VideoIconStyle = {
  internal: {
    paddingX: number;
    paddingY: number;
  };
  external: {
    paddingX: number;
    paddingY: number;
  };
  width: number;
  height: number;
  color: string;
  dataURL: string;
};

type VideoStyle = {
  playPauseOnDblClick: boolean;
  track: VideoTrackStyle;
  background: VideoBackgroundStyle;
  iconBackground: VideoIconBackgroundStyle;
  icon: VideoIconStyle;
};

type URLTransformerFunction = (url: string, node?: Konva.Node) => string;

type WeaveVideoProperties = {
  crossOrigin: ImageCrossOrigin;
  transform?: WeaveNodeTransformerProperties;
  urlTransformer?: URLTransformerFunction;
  style: VideoStyle;
};

type WeaveVideoNodeParams = {
  config: Partial<WeaveVideoProperties>;
};

type WeaveVideoOnVideoPlayEvent = {
  nodeId: string;
};

type WeaveVideoOnVideoPauseEvent = {
  nodeId: string;
};

type WeaveVideoOnVideoStopEvent = {
  nodeId: string;
};
----

== Parameters

For `WeaveVideoNodeParams`:

[cols="4*"]
|===
| Property | Type | Required | Description

| mono:[config] | mono:[Partial<WeaveVideoProperties>] | label:green[Required] | Config parameters for the node.
|===

For `WeaveVideoProperties`:

[cols="5*"]
|===
| Property | Type | Required | Default | Description

| mono:[crossOrigin] | mono:[ImageCrossOrigin] |  | mono:[anonymous] | Value of the property crossOrigin for the HTMLImageElement DOM element. Useful for CORS support.
| mono:[transform] | mono:[object] |  |  | Setup the transform properties for the image (if can be resized, rotated, anchors, etc.).
| mono:[urlTransformer] | mono:[URLTransformerFunction] |  |  | Setup a function that is called to transform the node URL, useful when rendering the stage server-side
| mono:[style] | mono:[VideoStyle] |  |  | Setup the style of the video
|===

== Default values

[source,ts]
----
const WEAVE_VIDEO_DEFAULT_CONFIG: WeaveVideoProperties = {
  crossOrigin: "anonymous",
  style: {
    // Allows to double click to play or pause the video
    playPauseOnDblClick: true,
    background: {
      color: "#C9C9C9FF",
      strokeWidth: 0,
      strokeColor: "#000000FF",
    },
    track: {
      // After the video ends playback it will reset
      resetOnEnd: true,
      // Video playback progress is only showed when node is hovered
      onlyOnHover: false,
      color: "#FF0033FF",
      height: 5,
    },
    iconBackground: {
      color: "#FFFFFFFF",
      strokeWidth: 0,
      strokeColor: "#000000FF",
    },
    icon: {
      internal: {
        paddingX: 12,
        paddingY: 8,
      },
      external: {
        paddingX: 20,
        paddingY: 20,
      },
      width: 24,
      height: 24,
      color: "#FFFFFFFF",
      dataURL:
        "data:image/png;base64,",
    },
  },
};
----

== Methods

=== getVideoState

[source,ts]
----
getVideoState(nodeInstance: WeaveElementInstance): WeaveVideoState | undefined
----

Function that when is called returns the state of the internal video of the defined node.

=== play

[source,ts]
----
play(videoId: string): void
----

Function that when is called start the video playback of the defined node.

=== pause

[source,ts]
----
pause(videoId: string): void
----

Function that when is called pauses the video playback of the defined node.

=== stop

[source,ts]
----
stop(videoId: string): void
----

Function that when is called stop the video playback of the defined node. Also reset the video to
it's start origin.

=== areVideosPlaying

[source,ts]
----
areVideosPlaying(): boolean
----

Function that when is called returns true if at least one video is playing back.

=== getVideoSource

[source,ts]
----
getVideoSource(videoId: string): HTMLVideoElement | undefined
----

Function that when is called returns video source for the defined node.

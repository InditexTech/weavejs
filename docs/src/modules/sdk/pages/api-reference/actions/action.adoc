...
The abstract https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/actions/action.ts[WeaveAction]
...
WeaveAction class uses internally a link:[Proxy], that allows us to intercept operations of the class, for example
...
| mono:[onPropsChange] | mono:[WeaveActionPropsChangeCallback] |  | A callback that is called when action properties changed.
|===

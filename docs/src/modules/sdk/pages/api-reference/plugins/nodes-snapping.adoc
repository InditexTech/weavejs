...
The https://github.com/InditexTech/weavejs/blob/main/code/packages/sdk/src/plugins/nodes-snapping/nodes-snapping.ts[WeaveNodesSnappingPlugin]
...
| mono:[config] | mono:[WeaveNodesSnappingPluginConfig] |  | Config parameters for the plugin.
|===
...
| mono:[snap.tolerance] | mono:[number] |  | mono:[5] | Tolerance in px to magnet-snap towards guides.
| mono:[persistence.enabled] | mono:[SnappingManagerPersistenceConfig] |  | mono:[true] | Defines if the guides need to be persisted.
| mono:[movement.delta] | mono:[number] |  | mono:[0.5] | Defines the delta in px when moving the guides via keyboard.
| mono:[movement.shiftDelta] | mono:[number] |  | mono:[10] | Defines the delta in px when moving the guides via keyboard with shift pressed.
| mono:[style] | mono:[SnappingManagerStyle] |  | mono:[DEFAULT_SNAPPING_MANAGER_CONFIG.style] | Defines the UI style of the guides.
| mono:[targetDistanceStyle] | mono:[GuideDistanceToTargetInfoStyle] |  | mono:[DEFAULT_SNAPPING_MANAGER_CONFIG.targetDistanceStyle] | Defines the UI style to show the distance from the selected target when creating / updating a custom guide.
| mono:[getStaticGuides] | mono:[WeaveGetStaticGuidesFunction] |  | mono:[undefined] | Function that define the static guides to render.
|===

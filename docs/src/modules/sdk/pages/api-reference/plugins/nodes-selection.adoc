...
type WeaveNodesSelectionMode = 'intersects' | 'contains';
...
| mono:[selectionMode] | mono:[WeaveNodesSelectionMode] | | mono:['intersects'] | Drag-selection (marquee) hit-test mode. `'intersects'` selects any node whose bounding box overlaps the selection rectangle (default, matches prior behavior). `'contains'` selects a node only if its bounding box is fully enclosed by the selection rectangle; for a group node, containment is checked against the group's own outer bounding box, not per child. Frame nodes are always selected by containment regardless of this setting.
|===

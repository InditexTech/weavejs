...
The Azure Web PubSub Store is store for Weave.js that leverages https://azure.microsoft.com/es-es/products/web-pubsub[Azure Web PubSub]
...
Check the xref:store-azure-web-pubsub:index.adoc[Azure Web PubSub store] package API reference
for an overview of how to use it both:

* xref:store-azure-web-pubsub:index.adoc#backend-side-usage[Server-side] or
* xref:store-azure-web-pubsub:index.adoc#frontend-side-usage[Frontend-side]
...
[source,ts]
----
include::example$build/stores/azure-web-pubsub-server.ts[]
----
...
const roomData: Uint8Array | undefined = undefined;

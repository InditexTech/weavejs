...
Check the xref:store-standalone:index.adoc[Standalone store] package API reference.
...
controller can call a https://nodejs.org/api/worker_threads.html[Worker] - to avoid blocking the
...
<4> Call the xref:sdk:api-reference/weave/index.adoc#exportnodesserverside[`exportNodesServerSide`] function on the Weave.js instance to perform the export, `nodes` can be
any node you want.

...
* *Brush tool* — it was no longer possible to draw a dot or very small stroke on the canvas. This is now fixed. (https://github.com/InditexTech/weavejs/issues/1088[#1088])
* *WeaveTextNode* — the textarea edit overlay was misaligned due to a hardcoded border offset and an inaccurate `scrollHeight` approximation. The overlay now tracks the text node position correctly. (https://github.com/InditexTech/weavejs/issues/1098[#1098])
...
. *Update custom store implementations* — add `async` and return `Promise<void>` from `connect()` and `disconnect()`.
. *Adopt the `imageFallback` API* — if you rely on image fallback data being stored in the Yjs document, configure the new `imageFallback` option on `WeaveImageNode` to persist fallback images in your own storage.
. *Update font loading functions* — if you use a `fontsConfig` function, update its signature to accept and use the `loadFontsFamilies` helper.
. *Remove `@syncedstore/core`* — replace any direct usage with Yjs APIs accessed via `store.getDoc()`.
. *Register new nodes and actions* (optional) — add `WeavePolygonNode` and `WeavePolygonToolAction` to unlock polygon drawing; no action needed if you do not want the feature.

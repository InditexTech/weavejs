...
```shell
npm install canvas ws y-protocols yjs @inditextech/weavejs-store-websockets
```
...
Then setup your server as needed, here you have an example of a simple Express server,
...
```ts
import { WeaveStoreWebsockets } from "@inditextech/weavejs-store-websockets/client"; // (1)

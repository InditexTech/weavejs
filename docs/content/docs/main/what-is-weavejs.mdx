...
to bridge React’s component model state for the Konva rendering process. This integration allows
developers to define nodes as Classes with specific life-cycle and we internally handle the rest.
This way we benefit from efficient, incremental updates and the internal React's renderer life-cycle
optimizations without the need to know React declaratively way (JSX).

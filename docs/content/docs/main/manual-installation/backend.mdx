...
Now that the Next.js custom server is completely set up, let's add the necessary Weave.js backend logic. For that, we will use the xref:api-reference:stores/websockets/index.adoc[Websockets store] library: 

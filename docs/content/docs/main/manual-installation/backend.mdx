...
Before you begin, ensure that you have completed the [manual installation guide](/docs/main/requirements) up to the *"Set up the backend project"* step.

## Step by step

To setup Weave.js backend over our Next.js project (on a single artifact), follow these steps:
...
For more details, refer to our [backend showcase implementation](https://github.com/InditexTech/weavejs-backend).
</Callout>
...
<Callout title="Express" type="note">
  This sections uses [Express](https://expressjs.com), a web framework for Node.js, as the underlying server for the custom server.
</Callout>
...
Now the Next.js custom server is completely set up, let's add the necessary Weave.js backend logic for that we will use the xref:api-reference:stores/websockets/index.adoc[Websockets store] library
...
In this case, we will use the file system as our persistence layer.
...
Finally, setup the Weave.js store handler.
In this case, we will set up the Websockets transport.

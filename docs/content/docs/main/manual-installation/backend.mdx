...
Then, set up the TypeScript configuration for the server:

- Set up the transpilation of the server files from TypeScript to JavaScript. Create a `nodemon.json` file on the root of your project with:
...
- Create a `next.config.mjs` file in the root of your project with:
...
Define the custom server logic by creating a file named `server.ts` in the root of your project with:

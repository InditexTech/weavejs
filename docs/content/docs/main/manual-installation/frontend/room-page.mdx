...
- In the `components/utils` folder, create a file named `constants.ts` with:
  ...
  - **`(5)`**: Define the custom plugins used in this example. By default the [WeaveProvider](/docs/react/api-reference/providers/weave-provider)
    pre-defines a set of plugins. In this case, which can be overriden or extended here.
...
Replace the content of the `lib/utils.ts` with:
...
In the `hooks` folder, create a file named `use-get-websockets-store.ts` with the following
content:

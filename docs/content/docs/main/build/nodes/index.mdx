...
The Comment node provides a way to let know the user where a comment has added to the room.
Also provides a mechanism to render an overlay to manage the comment and the comment replies.

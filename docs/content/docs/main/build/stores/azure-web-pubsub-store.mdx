...
```shell
npm install canvas @azure/web-pubsub @azure/web-pubsub-express y-protocols yjs @inditextech/weavejs-store-azure-web-pubsub
```
...
```ts
import { WeaveStoreAzureWebPubsub } from "@inditextech/weavejs-store-azure-web-pubsub/client"; // (1)

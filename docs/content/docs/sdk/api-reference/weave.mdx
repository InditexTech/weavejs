...
When `stopAtGroupId` is provided (a single group ID or an array of IDs), the traversal stops
at the direct child of any matching group — useful when a group edit context is active and
you need to resolve a hit node relative to the group boundary rather than the root.

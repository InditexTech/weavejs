...
<TypeTable
  type={{
    ...
    ["weaveConfig.renderer"]: {
      required: true,
      description:
        "The renderer to use to handle the real-time reconciliation of the shared state",
      type: "WeaveRendererBase",
    },
    ["weaveConfig.nodes"]: {
      description:
    ...
    ["weaveConfig.fonts"]: {
      description:
        "An array of fonts to be registered and used on the instance, or a preload function that dynamically loads font families and returns the resolved fonts",
      type: "WeaveFont[] | WeaveFontsPreloadFunction",
    },
    ["weaveConfig.logger"]: {
...
canvas side. With this information you can use a tool like [sharp](https://sharp.pixelplumbing.com/api-composite/)

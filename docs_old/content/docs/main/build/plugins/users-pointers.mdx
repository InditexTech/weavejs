...
const getUserBackgroundColor = (user: WeaveUser) => // [!code ++]
  getUserBackgroundColor(user?.name ?? "#000000") // [!code ++]

const getUserForegroundColor = (user: WeaveUser) => { // [!code ++]
  const bgColor = getUserBackgroundColor(user?.name ?? "#000000"); // [!code ++]
  return getContrastColor(bgColor); // [!code ++]
} // [!code ++]
...
const instance = new Weave({
  ...
  plugins: [
    ...
    new WeaveUsersPointersPlugin({ // [!code ++]
      config: { // [!code ++]
        getUser, // [!code ++]
        getUserBackgroundColor, // [!code ++]
        getUserForegroundColor, // [!code ++]
      }, // [!code ++]
    }), // [!code ++]
  ]
})

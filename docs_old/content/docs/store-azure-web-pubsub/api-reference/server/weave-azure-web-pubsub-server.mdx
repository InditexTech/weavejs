...
<TypeTable
  type={{
    ...
    eventsHandlerConfig: {
      description:
        "Azure Web PubSub event handler configuration. Use allowedEndpoints to restrict the CloudEvents origin and accessKey to verify the HMAC-SHA256 signature on every inbound event.",
    ...
    ["syncHostConfig.heartbeat.sendIntervalMs"]: {
      description:
        "Interval to send sync room heartbeat to clients, in miliseconds",
      type: "number",
      default: "2500",
    },
    ["syncHostConfig.resync.checkIntervalMs"]: {
      description:
        "Sync host resync request check interval, used to request a resync event to clients when sync host disconnect / reconnect, in miliseconds",
      type: "number",
      default: "5000",
    },
...
> [Azure Web PubSub IP ranges](https://learn.microsoft.com/en-us/azure/azure-web-pubsub/concept-service-internals)

  ...
  getMousePointer(point?: Vector2d) {
    ...
    if (intersectedNode) {
      ...
      if (node && node instanceof Konva.Group && node.getAttrs().containerId) {
        measureContainer = (node as Konva.Group).findOne(`#${node.getAttrs().containerId}`) as Konva.Group;
        container = node;
      }
    }

    console.log({ measureContainer, container, type: measureContainer?.getAttrs().nodeType });

    if (container?.getAttrs().nodeType !== "layer") {
      relativeMousePointer = measureContainer?.getRelativePointerPosition() ?? relativeMousePointer;
    }

    if (container?.getAttrs().nodeType === "layer") {
      relativeMousePointer = measureContainer?.getRelativePointerPosition() ?? { x: 0, y: 0 };
    }

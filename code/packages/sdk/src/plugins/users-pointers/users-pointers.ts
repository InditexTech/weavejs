...
export class WeaveUsersPointersPlugin extends WeavePlugin {
  ...
  getLayer() {
    const stage = this.instance.getStage();
    return stage.findOne(`#${WEAVE_USERS_POINTERS_LAYER_ID}`) as Konva.Layer | undefined;
  }
  ...
  private setUserMovementTimer(userPointer: WeaveUserPointer) {
    ...
    this.usersPointersTimers[userPointer.user] = setTimeout(() => {
      const userPointerNode = pointersLayer?.findOne(`#${userPointer.user}`) as Konva.Group | undefined;
  ...
  private renderPointers() {
    ...
    for (const userPointerKey of Object.keys(this.usersPointers)) {
      ...
      const userPointerNode = pointersLayer?.findOne(`#${userPointer.actualPos.user}`) as Konva.Group | undefined;

...
import type { WeaveStagePanningPlugin } from '@/plugins/stage-panning/stage-panning';

let clones: Konva.Node[] = [];
...
export abstract class WeaveNode implements WeaveNodeBase {
  ...
  setupDefaultNodeEvents(node: Konva.Node): void {
    ...
    if (isLocked) {
      ...
      const stage = this.instance.getStage();
      // const cloned: Record<string, Konva.Node | undefined> = {};
      ...
      node.on('dragstart', (e) => {
        ...
        if (e.evt?.altKey) {
          ...
          if (clone && !clones.find((c) => c === clone)) {
          ...
          setTimeout(() => {
            nodesSelectionPlugin?.setSelectedNodes(clones);
      ...
      const handleDragMove = (e: KonvaEventObject<DragEvent, Konva.Node>) => {
        ...
        if (this.isSelecting() && !realNodeTarget.getAttrs().isCloned) {
      ...
      node.on('dragend', (e) => {
        ...
        if (this.isSelecting()) {
          ...
          let moved = false;
          if (realNodeTarget.getAttrs().isCloned) {
            const parent: Konva.Container =
              realNodeTarget.getParent() as Konva.Container;
            const realParent = this.instance.getInstanceRecursive(parent);
            containerToMove = realParent;
          }
          ...
          if (realNodeTarget.getAttrs().isCloned) {
            clones = clones.filter((c) => c !== realNodeTarget);
          }
